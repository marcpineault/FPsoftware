    ...
    return (
      <div className="w-full">
        ...
        <input
          ...
          id={inputId}
          className={[
            'block w-full rounded-lg border bg-white px-3 py-2 text-sm text-text-primary',
            'placeholder:text-text-secondary/60',
            'transition-colors',
            'focus:outline-none focus:ring-2 focus:ring-offset-0',
            error
              ? 'border-negative focus:border-negative focus:ring-negative/30'
              : 'border-card-border focus:border-navy focus:ring-navy/30',
        ...
        {error && (
          <p id={`${inputId}-error`} className="mt-1 text-sm text-negative">
            {error}
          </p>
        )}
        {!error && hint && (
