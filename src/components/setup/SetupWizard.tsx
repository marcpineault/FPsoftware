...
import { estimateCppMonthlyAt65, getMarginalTaxRate } from '../../lib/calculations';
import type {
  CanadianProvince,
  EmploymentStatus,
  InsuranceType,
...
function StepInsights({ step, c }: { step: string; c: import('../../lib/types').Client }) {
  ...
  if (step === 'client') {
    if (age !== null) {
      insights.push({ label: 'Current age', value: `${age}` });
      const retAge = c.targetRetirementAge || 65;
      const yrs = Math.max(0, retAge - age);
      if (yrs > 0) insights.push({ label: 'Years to retirement', value: `${yrs}`, note: `at age ${retAge}` });
    }
  }

  if (step === 'income') {
    if (c.annualIncome > 0) {
      const marginal = getMarginalTaxRate(c.annualIncome, c.province);
      insights.push({ label: 'Marginal tax rate', value: `${(marginal * 100).toFixed(1)}%`, note: PROVINCE_NAMES[c.province] });
      const rrspLimit = Math.min(Math.round(c.annualIncome * 0.18), 32490);
      insights.push({ label: 'RRSP contribution limit', value: fmtC(rrspLimit), note: '18% of income (2025 max $32,490)' });
      const cppEst = estimateCppMonthlyAt65(c.annualIncome);
      insights.push({ label: 'Est. CPP at 65', value: `${fmtC(cppEst)}/mo`, note: `${fmtC(cppEst * 12)}/yr` });
      if (age !== null && c.targetRetirementAge) {
        const yrsToRet = Math.max(0, c.targetRetirementAge - age);
        const monthlyIncome = Math.round(c.annualIncome / 12);
        insights.push({ label: 'Monthly income', value: fmtC(monthlyIncome) });
        if (yrsToRet > 0) insights.push({ label: 'Years to retirement', value: `${yrsToRet}` });
      }
    }
    if (c.hasSpouse && c.spouse?.annualIncome) {
      const household = c.annualIncome + c.spouse.annualIncome;
      insights.push({ label: 'Household income', value: fmtC(household) });
    }
  }

  if (step === 'expenses') {
    if (c.monthlyExpenses > 0 && c.annualIncome > 0) {
      const annualExp = c.monthlyExpenses * 12;
      const savingsRate = Math.round(((c.annualIncome - annualExp) / c.annualIncome) * 100);
      insights.push({ label: 'Annual expenses', value: fmtC(annualExp) });
      insights.push({ label: 'Gross savings rate', value: `${savingsRate}%`, note: savingsRate < 10 ? 'Low — consider increasing' : savingsRate >= 20 ? 'Strong savings rate' : 'Moderate' });
      const retExpenses = Math.round(c.monthlyExpenses * c.projectionParams.retirementSpendingRate);
      insights.push({ label: 'Retirement expenses', value: `${fmtC(retExpenses)}/mo`, note: `${Math.round(c.projectionParams.retirementSpendingRate * 100)}% of current` });
    }
  }

  if (step === 'assets') {
    const totalRegistered = (c.rrspBalance || 0) + (c.tfsaBalance || 0) + (c.fhsaBalance || 0);
    const totalAll = totalRegistered + (c.nonRegisteredInvestments || 0);
    const spouseAssets = c.hasSpouse ? ((c.spouse?.rrspBalance || 0) + (c.spouse?.spousalRrspBalance || 0) + (c.spouse?.tfsaBalance || 0)) : 0;
    if (totalAll > 0 || spouseAssets > 0) {
      insights.push({ label: 'Total registered', value: fmtC(totalRegistered), note: 'RRSP + TFSA + FHSA' });
      if (c.nonRegisteredInvestments > 0) insights.push({ label: 'Non-registered', value: fmtC(c.nonRegisteredInvestments) });
      insights.push({ label: 'Total investments', value: fmtC(totalAll + spouseAssets), note: spouseAssets > 0 ? 'Including spouse' : undefined });
      const annualContrib = (c.rrspAnnualContribution || 0) + (c.tfsaAnnualContribution || 0) + (c.fhsaAnnualContribution || 0);
      if (annualContrib > 0) insights.push({ label: 'Annual contributions', value: fmtC(annualContrib) });
    }
  }

  if (step === 'debts') {
    const netHome = (c.primaryResidenceValue || 0) - (c.mortgageBalance || 0);
    if (c.primaryResidenceValue > 0) {
      insights.push({ label: 'Home equity', value: fmtC(netHome) });
      if (c.mortgageBalance > 0 && c.primaryResidenceValue > 0) {
        const ltv = Math.round((c.mortgageBalance / c.primaryResidenceValue) * 100);
        insights.push({ label: 'LTV ratio', value: `${ltv}%`, note: ltv > 80 ? 'Above 80% — CMHC insurance required' : 'Conventional mortgage' });
      }
    }
    if (c.mortgageBalance > 0 && c.mortgageRate > 0 && c.mortgageAmortizationYears > 0) {
      const monthlyRate = c.mortgageRate / 12;
      const totalPayments = c.mortgageAmortizationYears * 12;
      const payment = c.mortgageBalance * (monthlyRate * Math.pow(1 + monthlyRate, totalPayments)) / (Math.pow(1 + monthlyRate, totalPayments) - 1);
      insights.push({ label: 'Mortgage payment', value: `${fmtC(Math.round(payment))}/mo` });
    }
    const totalDebt = (c.mortgageBalance || 0) + (c.otherDebts || 0);
    if (totalDebt > 0) insights.push({ label: 'Total debt', value: fmtC(totalDebt) });
    const totalAssets = (c.rrspBalance || 0) + (c.tfsaBalance || 0) + (c.nonRegisteredInvestments || 0) + (c.primaryResidenceValue || 0);
    if (totalAssets > 0 || totalDebt > 0) {
      insights.push({ label: 'Net worth', value: fmtC(totalAssets - totalDebt) });
    }
  }

  if (step === 'benefits') {
    if (c.projectionParams.estimatedCppMonthly > 0) {
      const cppAnnual = c.projectionParams.estimatedCppMonthly * 12;
      insights.push({ label: 'CPP annual (at 65)', value: fmtC(cppAnnual) });
      if (c.projectionParams.cppStartAge < 65) {
        const reduction = (65 - c.projectionParams.cppStartAge) * 12 * 0.006;
        insights.push({ label: `Early CPP penalty (age ${c.projectionParams.cppStartAge})`, value: `-${(reduction * 100).toFixed(1)}%`, note: fmtC(Math.round(c.projectionParams.estimatedCppMonthly * (1 - reduction)) * 12) + '/yr' });
      } else if (c.projectionParams.cppStartAge > 65) {
        const bonus = (c.projectionParams.cppStartAge - 65) * 12 * 0.007;
        insights.push({ label: `Deferred CPP bonus (age ${c.projectionParams.cppStartAge})`, value: `+${(bonus * 100).toFixed(1)}%`, note: fmtC(Math.round(c.projectionParams.estimatedCppMonthly * (1 + bonus)) * 12) + '/yr' });
      }
    }
    // OAS estimate
    const oasMonthly = 727; // 2025 max
    const oasAnnual = oasMonthly * 12;
    insights.push({ label: 'Max OAS (2025)', value: `${fmtC(oasMonthly)}/mo`, note: `${fmtC(oasAnnual)}/yr at age 65` });
    if (c.projectionParams.oasStartAge > 65) {
      const oasBonus = (c.projectionParams.oasStartAge - 65) * 12 * 0.006;
      insights.push({ label: `Deferred OAS bonus`, value: `+${(oasBonus * 100).toFixed(1)}%` });
    }
    if (c.pensionType === 'db' && c.pensionDetails?.annualBenefitEstimate) {
      insights.push({ label: 'DB pension income', value: fmtC(c.pensionDetails.annualBenefitEstimate) + '/yr' });
    }
  }

  if (step === 'insurance') {
    if (c.annualIncome > 0) {
      const quickLifeNeed = c.annualIncome * 10;
      insights.push({ label: 'Rule of thumb (10x income)', value: fmtC(quickLifeNeed), note: 'Life insurance starting point' });
      if (c.hasLifeInsurance && c.lifeInsuranceDetails?.coverageAmount) {
        const coverage = c.lifeInsuranceDetails.coverageAmount;
        const multiple = (coverage / c.annualIncome).toFixed(1);
        insights.push({ label: 'Current coverage', value: `${multiple}x income`, note: fmtC(coverage) });
      }
      insights.push({ label: 'Disability target (60%)', value: fmtC(Math.round(c.annualIncome * 0.6 / 12)) + '/mo' });
      insights.push({ label: 'CI target (2x income)', value: fmtC(c.annualIncome * 2) });
    }
  }
  ...
  return (
    <div className="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3">
      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Insights</p>
